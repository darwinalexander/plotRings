...
plotRings1 <- function(year, trwN, trwS = NA_real_,
                      ...
                      x.rings = "none", col.x.rings = "red", lwd = 1,
                      xy.lim = NULL,
                      main = "Annual tree growth",
                      species.name = NA,
  ...
  # Accumulative trw.means x 2   (To calculate directly the accumulative diameter)
  TRW$trw.acc.x2 <- TRW$trw.acc*2 
  
  # Accumulative trw.means - MAI
  #TRW$year.seq  <- seq(1, length(year), 1)
  #TRW$trw.acc.MAI <- TRW$trw.acc/TRW$year.seq
  
  TRW$year.seq  <- seq(1, length(TRW[,1]), 1)
  TRW$trw.acc.MAI <- TRW$trw.acc/TRW$year.seq
  ...
  # Acummulative BAI (inside out) - MAI
  TRW$bai.acc.MAI <-  TRW$bai.acc/TRW$year.seq 
  
  # Individual BAI (inside out)
  TRW$bai.ind  <-c(TRW$bai.acc[1], TRW$bai.acc[2:nrow(TRW)] - TRW$bai.acc[1:nrow(TRW)-1])  
  
   # Accumulative Basal area increment - CAI
  TRW$bai.CAI <- ffcsaps(TRW$bai.ind, x = as.numeric(row.names(TRW)), nyrs = 12)
  ...
  if (animation == TRUE) {
    ...
    for (i in 1:length(x)) {
      ...
      symbols(y = y[1:i], x = if(length(x) > 0) y[1:i] else x[1:i],
              ...
              xlab='Width [mm]', ylab='Width [mm]', main=mtext(bquote(~bold(.(main))),
              ...
              else if(x.rings == "none") cols, lwd = lwd) 
  ...
  # Without animation
    ...
    symbols( y = y, x = if(length(x) > 0) y else x,
             ...
             xlab='Width [mm]', ylab='Width [mm]', main=mtext(bquote(~bold(.(main))), line=1.5,adj=0.5, 
  ...
  if (saveGIF == TRUE) {
    ...
    saveGIF({
      ani.options(interval = sys.sleep, nmax = 50, ani.width = 1000, ani.height = 1000, res=1200)
      ...
      for (i in 1:length(x)) {
        ...
        # year labels
        #if(year.labels == TRUE) legend('topright', legend=year[i], bty="n", inset = 0.01, cex=2)
        if(year.labels == TRUE) title(sub=year[i], cex =2)
      }
