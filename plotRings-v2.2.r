...
plotRings2 <- function(trwN, trwS = NA_real_,
...
TRW.NAfill <- eof(TRW, recursive=TRUE) #  Convert to RSEOF - "Recursively Subtracted Empirical Orthogonal Functions" 
...
TRW[,1:4] <- TRW.NAfill_rcn[, 1:4]  # Replacing data to the original data.frame



#data arrangement for the four cardinal axes

data <- data.frame(N = cumsum(TRW[,4])*-1,
                   S = cumsum(TRW[,3]), 
                   E = cumsum(TRW[,2])*-1,
                   W = cumsum(TRW[,1]) )
...
data <- t(data)
colnames(data) <-  rownames(TRW)          # ring-factor
...
print(sapply(res[1,], function(a) pi * a[1] * a[2]))
